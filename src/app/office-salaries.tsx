...
const normalizeOfficeSalary = (s: any): OfficeSalary => ({
  id: Number(s.id ?? 0),
  name: String(s.name ?? ''),
  salary: Number(s.salary ?? 0),
  type: String(s.type ?? 'monthly'),
  date: s.date ? String(s.date) : undefined,
  worksite_id: s.worksite_id ? Number(s.worksite_id) : null,
  worksite: s.worksite,
});
...
export default function OfficeSalariesPage() {
  ...
  const [salaries, setSalaries] = useState<OfficeSalary[]>([]);
  const [search, setSearch] = useState('');
  const [formOpen, setFormOpen] = useState(false);
  const [isEditing, setIsEditing] = useState(false);
  const [selectedSalary, setSelectedSalary] = useState<OfficeSalary | null>(null);
  ...
  const [selectedViewItem, setSelectedViewItem] = useState<OfficeSalary | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const resp = await fetch(`${API_BASE_URL}/office-salaries`, {
          headers: { Accept: 'application/json', ...authHeader },
        });
        const data = await resp.json();
        const normalized = Array.isArray(data) ? data.map(normalizeOfficeSalary) : (data.data || []).map(normalizeOfficeSalary);
  ...
  const filteredSalaries = useMemo(() => {
    return salaries.filter((s) => {
      ...
      const matchesSite = siteFilter ? s.worksite_id === siteFilter : true;
  ...
  const openEditSalary = (salary: OfficeSalary) => {
    setSelectedSalary(salary);
    setIsEditing(true);
    const parsedDate = salary.date ? new Date(salary.date) : new Date();
    setFormValues({ name: salary.name, salary: String(salary.salary ?? 0), type: salary.type, date: salary.date ?? '', worksite_id: salary.worksite_id ?? null });
  ...
  const handleSaveSalary = async () => {
    ...
    const url = isEditing && selectedSalary ? `${API_BASE_URL}/office-salaries/${selectedSalary.id}` : `${API_BASE_URL}/office-salaries`;
    ...
    try {
      ...
      if (!response.ok) throw new Error(saved.message || 'Unable to save office salary');

      const normalized = normalizeOfficeSalary(saved);
      setFormOpen(false);
      setSalaries((prev) => (isEditing && selectedSalary ? prev.map((p) => (p.id === selectedSalary.id ? normalized : p)) : [normalized, ...prev]));
      setSuccessMessage(isEditing ? 'Salary Updated Successfully!' : 'Salary Added Successfully!');
    } catch (error) {
      Alert.alert('Error', error instanceof Error ? error.message : 'Unable to save office salary');
    }
  };
  ...
  const handleDeleteSalary = async (salaryId: number) => {
    ...
    const res = await fetch(`${API_BASE_URL}/office-salaries/${salaryId}`, {
      method: 'DELETE',
    ...
    if (!res.ok) {
      ...
      Alert.alert('Delete failed', body.message || 'Unable to delete office salary');
      return;
    }
          ...
          <ScrollView style={styles.tableBody} showsVerticalScrollIndicator={false}>
            {filteredSalaries.map((salary) => (
              <View key={salary.id} style={styles.tableRow}>
                <Text style={[styles.rowCell, { width: 60, minWidth: 60, flex: 0 }]} numberOfLines={1}>{salary.id}</Text>
                <Text style={styles.rowCell} numberOfLines={1}>{salary.name}</Text>
                <Text style={[styles.rowCell, { width: 120, minWidth: 120, flex: 0 }]} numberOfLines={1}>{salary.worksite?.name ?? 'Unassigned'}</Text>
                <Text style={[styles.rowCell, { width: 120, minWidth: 120, flex: 0 }]} numberOfLines={1}>{salary.salary}</Text>
                <Text style={[styles.rowCell, { width: 120, minWidth: 120, flex: 0 }]} numberOfLines={1}>{salary.type}</Text>
          ...
          <View style={styles.formOverlay}>
            <View style={styles.formCard}>
              ...
              <ScrollView style={styles.formBody} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.fieldRow}>
                  ...
                  <View style={styles.typeList}>
                    {['monthly', 'annual', 'hourly'].map((typeOption) => (
                      <Pressable
                        key={typeOption}
                        style={[styles.typeOption, formValues.type === typeOption && styles.typeOptionSelected]}
                        onPress={() => setFormValues((prev) => ({ ...prev, type: typeOption }))}
                      >
                        <Text style={[styles.typeOptionText, formValues.type === typeOption && styles.typeOptionTextSelected]}>{typeOption}</Text>
                      </Pressable>
                    ))}
                  </View>
                </View>
                ...
                {Platform.OS === 'web' ? (
                  <View style={styles.fieldRow}>
                    ...
                    <Pressable style={[styles.pill, { backgroundColor: theme.background }]} onPress={() => { webDateInputRef.current?.showPicker?.(); webDateInputRef.current?.click(); }}>
                      <Text style={styles.pillText}>{formValues.date || salaryDate.toISOString().split('T')[0]}</Text>
                    </Pressable>
                    <input
                      ref={webDateInputRef}
                      type="date"
                  ...
                  <>
                    <View style={styles.fieldRow}>
                      ...
                      <Pressable style={[styles.pill, { backgroundColor: theme.background }]} onPress={() => setShowDatePicker(true)}>
                        <Text style={styles.pillText}>{salaryDate.toLocaleDateString('en-US', { day: '2-digit', month: 'short', year: 'numeric' })}</Text>
                      </Pressable>
                    </View>
                    {showDatePicker && (
                      <DateTimePicker
                ...
                <Pressable style={styles.saveButton} onPress={handleSaveSalary}><Text style={styles.saveText}>Save Salary</Text></Pressable>
              </ScrollView>
            </View>
          </View>
        )}
                ...
                {selectedViewItem && (
                  <>
                    <View style={styles.detailRow}>
                      <ThemedText type="smallBold" style={styles.detailLabel}>Employee Name</ThemedText>
...
const createStyles = (theme: ReturnType<typeof useTheme>) =>
  StyleSheet.create({
    ...
    selectInput: { width: '100%', padding: Spacing.two, borderRadius: 24, backgroundColor: theme.background, borderWidth: 1, borderColor: theme.backgroundSelected },
    selectText: { color: theme.text, fontSize: 13 },
    statusOptions: { borderRadius: 20, overflow: 'hidden', marginTop: Spacing.one, backgroundColor: theme.backgroundSelected },
    statusOption: { paddingVertical: Spacing.one, paddingHorizontal: Spacing.three, borderRadius: 20, backgroundColor: theme.backgroundSelected },
    statusOptionText: { color: theme.text, fontSize: 13 },
    ...
    typeList: { flexDirection: 'row', gap: Spacing.two, flexWrap: 'wrap' },
    typeOption: { paddingVertical: Spacing.one, paddingHorizontal: Spacing.three, borderRadius: 20, backgroundColor: theme.backgroundSelected },
    typeOptionSelected: { backgroundColor: '#0f172a' },
    typeOptionText: { color: theme.text, fontSize: 13 },
