...
target_file = r'c:\xampp1\htdocs\MyFirstProject (1)\MyFirstProject\src\app\bid-bonds.tsx'
...
# Update handleSave (already has formValues.duration_date, change it to fallback to selectedDate)
content = content.replace("duration_date: formValues.duration_date,",
...
date_input_jsx = """<Text style={[styles.label, { color: theme.textSecondary }]}>Date (Duration)</Text>
                {Platform.OS === 'web' ? (
...
content = re.sub(
    r"<Text style=\{\[styles\.label, \{ color: theme\.textSecondary \}\]\}>Date \(Duration\)</Text>\s*<TextInput.*?duration_date: v \}\)\)} />",
    date_input_jsx,
    content,
    flags=re.DOTALL
)
